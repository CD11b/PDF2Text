...
class LineCollector:
    ...
    def process(self, group: list[Span], ctx: SpanContext, decision: Decision) -> list[Span]:
    ...
    @staticmethod
    def _log(decision: Decision, merged: Span, ctx: SpanContext) -> None:
        logger.log(decision.action.log_level,
                   "%s [%s - CASE: %s]: %s, %s",
                   decision.action.action_label,
                   decision.handler_name, decision.reason,
                   merged if decision.action.log_verbose else merged.span.text,
                   ctx if decision.action.log_verbose else "")

    @staticmethod
    def _aggregate_line_group(line_group: list[Span], ctx: SpanContext) -> Span:
        """Merge lines of a line group into a single StyledLine."""

        first_group, last_group = line_group[0], line_group[-1]

        line = Span.create(text=' '.join(line.text for line in line_group if line.text.strip()),
        ...
        return ClassifiedSpan.create(line, ctx)